import { forwardRef, useEffect, useId, useLayoutEffect, useState, useImperativeHandle, useRef } from 'react'
...
interface PlumberWordmarkProps {
  className?: string
  /** Letters that follow the wrench-P (default "LUMBER" -> PLUMBER). */
  suffix?: string
  /** Rendered height in px. */
  height?: number
  /** false renders the static, tightened final state (no load animation, no replay). */
  animated?: boolean
}
...
const PlumberWordmark = forwardRef<PlumberWordmarkHandle, PlumberWordmarkProps>(
  ({ className, suffix = 'LUMBER', height = 50, animated = true }, ref) => {
    // Unique gradient ids: several wordmarks can be on the page, and a
    // display:none instance (mobile header) would otherwise break url(#id)
    // references for the others.
    const uid = useId().replace(/:/g, '')
    const id = (name: string) => `pw-${name}-${uid}`
    const textRef = useRef<SVGTextElement>(null)
    // Fit the viewBox to the real text width (Georgia metrics vary by letter/platform).
    const [textWidth, setTextWidth] = useState(suffix.length * 80)
    useLayoutEffect(() => {
      const w = textRef.current?.getComputedTextLength()
      if (w) setTextWidth(w)
    }, [suffix])
    const viewWidth = TEXT_X + textWidth + 6
    ...
    useEffect(() => {
      ...
      reducedMotionRef.current = mql.matches || !animated
    ...
    return (
      <div
        className={`inline-block select-none ${animated ? 'cursor-pointer' : ''} ${className ?? ''}`}
        onClick={animated ? replay : undefined}
        role="img"
        aria-label={`P${suffix.toLowerCase()}`}
      >
        <svg
          viewBox={`0 0 ${viewWidth} 118`}
          className="block"
          style={{ overflow: 'visible', height: `${height}px`, width: 'auto' }}
        >
          <defs>
            ...
            <linearGradient id={id('body')} gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="54" y2="0">
              <stop offset="0%" stopColor="#3b4653" />
              <stop offset="22%" stopColor="#8797a8" />
              <stop offset="45%" stopColor="#f1f5f9" />
              <stop offset="58%" stopColor="#b7c3cf" />
              <stop offset="100%" stopColor="#28313b" />
            </linearGradient>
            <linearGradient id={id('body-length')} gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="0" y2="100">
              <stop offset="0%" stopColor="#ffffff" stopOpacity="0.12" />
              <stop offset="50%" stopColor="#ffffff" stopOpacity="0" />
              <stop offset="100%" stopColor="#000000" stopOpacity="0.18" />
            </linearGradient>
            <radialGradient id={id('nut')} cx="32%" cy="28%" r="75%">
            ...
            <filter id={id('shadow-blur')} x="-50%" y="-50%" width="200%" height="200%">
          ...
          <g transform={`translate(6 ${CAP_TOP}) scale(${CAP_HEIGHT / 100})`}>
            ...
            <g fill={`url(#${id('body')})`} stroke="#0d1013" strokeWidth="0.8">
            ...
            <g transform="translate(15 54)">
              <circle r="9.5" fill={`url(#${id('body')})`} stroke="#0f1a24" strokeWidth="0.8" />
            ...
            <g ref={nutRef}>
              <polygon points={hexPoints(31, 32, 12.5)} fill={`url(#${id('nut')})`} stroke="#4a2e0f" strokeWidth="1" />
              <polygon
            ...
            <g ref={dripsRef}>
              <g ref={drip1Ref} transform="translate(26 45)">
                <path d="M0 0 C -1.6 3.4, -1.6 6.8, 0 8 C 1.6 6.8, 1.6 3.4, 0 0 Z" fill={`url(#${id('water')})`} />
              </g>
              <g ref={drip2Ref} transform="translate(32 46.5)">
                <path d="M0 0 C -1.3 3, -1.3 5.8, 0 6.7 C 1.3 5.8, 1.3 3, 0 0 Z" fill={`url(#${id('water')})`} />
              </g>
              <g ref={drip3Ref} transform="translate(29 44.5)">
                <path d="M0 0 C -1.1 2.5, -1.1 5, 0 5.7 C 1.1 5, 1.1 2.5, 0 0 Z" fill={`url(#${id('water')})`} />
              </g>
            </g>
